...
describe("Combine logics tests", () => {
  ...
  test("for looping in array with <include /> as child and include attr replacement", async () => {
    ...
    vi.spyOn(HtmlParser.prototype, "readFile").mockImplementation(() => {
      const card = `{undefinedFunction()}<a href="{user.link.href}" class="card">{user.link.name} {user.firstName} {user.lastName}'s page</a>`;
      return card;
    });
    ...
    expect(parserTest.transform(input)).toBe(
      `{undefinedFunction()}<a href="https://example.com/john" class="card">link to john doe's page</a>{undefinedFunction()}<a href="https://example.com/jannet" class="card">link to jannet foe's page</a>`
    );
  });
