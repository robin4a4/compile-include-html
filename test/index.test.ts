...
describe("<for /> tests", () => {
  test.skip("for loop without context", async () => {
  ...
  test.skip("for loop with context", async () => {
  ...
  test("for looping in array with <include /> as child", async () => {
    const input = `<div>\n  <for condition="const userContext of array"><include src='card.html' with='user: userContext;'></include></for>\n</div>`;
    vi.spyOn(Includer.prototype, "readFile").mockImplementation(() => {
      const card = `<div class="card">First name: {user.firstName}, Last name: {user.lastName}</div>`;
      return card;
    });
    ...
    expect(parserTest.transform(input)).toBe(
      `<div>\n  <div class="card">First name: john, Last name: doe</div><div class="card">First name: jannet, Last name: foe</div>\n</div>`
    );
  });
});
